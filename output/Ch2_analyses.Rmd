...
bp3 <- df2 %>%
  ...
  geom_boxplot(show.legend = FALSE) +
  xlab("Severity (% Gross Defoliation)") +
  ylab('Subcanopy CWM leaf mass per area (g/cm2)') + 
  scale_fill_manual(values = c("#000000", "#009E73", "#0072B2", "#D55E00")) + 
  facet_grid(~Year)
...
# lmeCondVAI2 <- lmer(Subplot_mean_Cond ~ mean_vai*Year + (1|Replicate), data = Asat_VAI)
...
gs_plot <- ggplot(Asat_VAI_noNAs, aes(x = mean_vai, y = Subplot_mean_Cond, colour = Year)) +
      geom_point(size = 2.5) +
      theme_classic(base_size = 13) +
      geom_smooth(data = cbind(Asat_VAI_noNAs, pred = predict(lmCondVAI)), aes(y = pred), method='lm', level=0.95, size = 1, na.rm=TRUE) +  # adding predicted line from mixed model 
  xlab("Canopy VAI (unitless)") +
  ylab('Subcanopy CWM leaf stomatal conductance (mol' ~CO[2]~ m^-2~s^-1*')')  # adding predicted line from mixed model 
gs_plot
...
with(npp, LSD.test(log_NPP, disturbance_severity:Year,48,0.286, console = TRUE))
...
bpx1 <- npp %>%
  ...
  geom_boxplot(show.legend = FALSE) +
  xlab("Severity (% Gross Defoliation)") +
  ylab(expression(paste("log ANPP" [w], " ( ",kgC," ",ha^-1," ",year^-1,")"))) +
  scale_fill_manual(values = c("#000000", "#009E73", "#0072B2", "#D55E00")) +
  facet_grid(~Year)
...
## NPP-VAI LME Model
lmeNPPVAI <- lmer(log_NPP ~ mean_vai*Year + (1|Replicate), data = npp_vai)
summary(lmeNPPVAI)
# show summary
...
m_asat_npp <- lm(log_NPP ~ Subplot_mean_Asat*Year, data = traits_npp)
summary(m_asat_npp)
# show summary
...
m_cond_npp <- lm(log_NPP ~ Subplot_mean_Cond*Year, data = traits_npp)
summary(m_cond_npp)
# show summary
...
# LMA 
p27 <- traits_npp %>%
  ggplot(aes(x = mean_lma, y = log_NPP, color=Year)) + 
  theme_classic(base_size = 15) + 
  geom_point(show.legend = TRUE, size=3) +
  xlab("Subcanopy LMA") +
  ylab("log Subcanopy ANPPw") 
...
m_lma_npp <- lm(log_NPP ~ mean_lma*Year, data = traits_npp)
summary(m_lma_npp)
# show summary
...
m_rendvi_npp <- lm(log_NPP ~ mean_rendvi*Year, data = traits_npp)
summary(m_rendvi_npp)
# show summary
sjPlot::tab_model(m_rendvi_npp)


## run full analysis WITH interactions between each leaf trait and year
lmeNPPtraitsInt <- lmer(log_NPP ~ Subplot_mean_Asat*Year + Subplot_mean_Cond*Year + mean_lma*Year + mean_rendvi*Year + Year + (1|Replicate), data = traits_npp)
summary(lmeNPPtraitsInt)

## all leaf traits, additive only (no interactions)
lmeNPPtraits1 <- lmer(log_NPP ~ Subplot_mean_Asat + Subplot_mean_Cond + mean_lma + mean_rendvi + Year + (1|Replicate), data = traits_npp)
summary(lmeNPPtraits1)

lmeNPPtraits2 <- lmer(log_NPP ~ Subplot_mean_Asat + Subplot_mean_Cond + mean_rendvi + Year + (1|Replicate), data = traits_npp)
summary(lmeNPPtraits2)

lmeNPPtraits3 <- lmer(log_NPP ~ Subplot_mean_Cond + mean_rendvi + Year + (1|Replicate), data = traits_npp)
